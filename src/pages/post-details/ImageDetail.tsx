...
export default function ImageDetail() {
  ...
  return (
    <div className="flex w-full min-h-screen bg-black text-white">
      ...
      <button
        onClick={() => navigate(-1)}
        className="absolute top-6 left-6 z-50 bg-black bg-opacity-70 text-white w-10 h-10 rounded-full flex items-center justify-center hover:bg-opacity-100"
      >
        ←
      </button>
          ...
          <div className="flex space-x-2 overflow-x-auto bg-black bg-opacity-70 px-4 py-2 rounded-xl max-w-[80%]">
            {post.images.map((img, index) => (
              <img
                ...
                className={`w-14 h-14 object-cover rounded-md cursor-pointer transition-opacity duration-200 ${
                  index === currentIndex ? 'opacity-100 border-2 border-white' : 'opacity-60'
                }`}
              />
            ))}
          </div>
      ...
      {/* 右侧信息 + 评论 */}
      <aside className="w-[360px] p-4 bg-white text-black overflow-y-auto border-l border-gray-200">
        {/* 作者信息 */}
